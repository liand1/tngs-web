...
// 查询字典数据列表
export function getDictDataPage(params: DictDataPageReqVO): Promise<PageResult<DictDataVO>> {
  return defHttp.get({ url: '/system/dict-data/page', params })
}


// 根据dictType查询字典数据列表
export function getDictDataPageToDictType(dictType: string): Promise<PageResult<DictDataVO>> {
  const params = {
    label: '',
    dictType: dictType,
    status: 0,

...
export function getAuthCache<T>(key: BasicKeys) {
  ...
  // const isLocal = userStore.getIsLocal
  // const fn = isLocal ? Persistent.getLocal : Persistent.getSession

  const sessionCache = Persistent.getSession(key)

  if (sessionCache) return sessionCache as T

  const localCache = Persistent.getLocal(key)

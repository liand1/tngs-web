import { findSampleChecktypeList } from '@/api/lims/statscenter';
...
export async function updateChartData(
  ...
  try {
    ...
    const res = await findSampleChecktypeList({
...
function convertResToSampleChartData(res: Record<string, SampleStatsRespVO[]>): SampleChartDataItem[] {
  const sampleTypeMap = {
    ...
    1: { name: "tNGS一步法呼吸道", field: "lung-lavage", color: "#F5515F" },
    2: { name: "tNGS一步法结核", field: "sputum", color: "#5D9CEC" },
  };
  // 颜色缓存

...
const updateChart = () => {
  ...
  const colors = data.map((item) => {
    if (item.isPositive && item.isSuspected) {
      return "#945DF2"; // 阳性&疑似
    } else if (item.isPositive) {
      return "#F5515F"; // 阳性
    } else if (item.isSuspected) {
      return "#5D9CEC"; // 疑似
    } else {
      return "#E8EAEC"; // 其他
    }
  ...
  if (activeChartType.value === CHART_TYPES.BAR) {
    ...
    const option: echarts.EChartsOption = {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          if (pathogen.isPositive && pathogen.isSuspected) {
            resultType = "阳性&疑似";
          } else if (pathogen.isPositive) {
            resultType = "阳性";
          } else if (pathogen.isSuspected) {
            resultType = "疑似";
          }
          ...
          //   <span style="display: inline-block; width: 8px; height: 8px; border-radius: 50%; background-color: #1890FF; margin-right: 5px;"></span>
          //   <span style="flex: auto;">样本类型: </span>
          //   <span>${pathogen.sampleTypes.join(", ")}</span>
          // </div>`;
    ...
    const pieData = data.map((item, index) => {
      return {
        ...
        value: item.value,
        itemStyle: {
          color: colorRandom[index % colorRandom.length],
        },
      };
    });
    ...
    const option: echarts.EChartsOption = {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          // // 添加结果类型
          // tooltipText += `<div style="display: flex; align-items: center; margin: 5px 0;font-size:12px; color:#000000D9;min-width: 150px;">
          //   <span style="display: inline-block; width: 8px; height: 8px; border-radius: 50%; background-color: #52C41A; margin-right: 5px;"></span>
          //   <span style="flex: auto;">结果类型: </span>

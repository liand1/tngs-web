          ...
          <template #description>
            <div class="step-box">
              <a-button
                ...
                v-if="isSecondReviewPassed(item) === 2"
              ...
              <a-tooltip placement="top">
                <template #title>
                  <span>备注:{{item.action}}</span>
                </template>
                <div class="descUnpass" v-if="(item.processStatus == 7 ||  item.processStatus == 9 || item.processStatus == 16) && item.action">备注:{{item.action}}</div>
              </a-tooltip>
              <a-tooltip placement="top">
                <template #title>
                  <span>备注:{{item.action}}</span>
                </template>
                <div class="descPass" v-if="(item.processStatus == 6 || item.processStatus == 8 || item.processStatus == 15) && item.action">备注:{{item.action}}</div>
              </a-tooltip>
            </div>
          </template>
        </a-step>
...
import { StepModel, sampleFlowListModel } from "./model";
...
const stepsFlowList = [
  ...
  initFlowList(ProcessStatusEnum.AGAIN_REPORT_PENDING),
  initFlowList(ProcessStatusEnum.AGAIN_REPORT_ISSUED),
  initFlowList(ProcessStatusEnum.AGAIN_REPORT_ISSUED_FAILED),
];
...
watch(
  ...
  (newValue) => {
    // console.log("props.findAnalysisReportDetails", newValue);
    ...
    if (newValue?.sampleFlowList) {
      ...
      steps.value.stepslist = [
        ...
        ...noAudit(mergedList),
        //重出报告
        ...againReportIssued(mergedList),
      ].sort((a, b) => a.processStatus - b.processStatus);

      // console.log("steps.value.stepslist", steps.value.stepslist);
...
function twoReview(mergedList: any) {
  ...
  if (group.length > 0) {
    ...
    if (notZero) {
      ...
      keep = [
        group.find(
          (obj) => obj.processStatus === ProcessStatusEnum.SECOND_REVIEW_PASSED
        )!,
      ];
    }
  }
...
//重出报告
function againReportIssued(mergedList: any) {
  ...
  const group = mergedList.filter(
    (item) =>
      item.processStatus === ProcessStatusEnum.AGAIN_REPORT_PENDING ||
      item.processStatus === ProcessStatusEnum.AGAIN_REPORT_ISSUED ||
      item.processStatus === ProcessStatusEnum.AGAIN_REPORT_ISSUED_FAILED
  );

  if (group.length > 0) {
    const errFlow = group.find((item) => item.processStatus === ProcessStatusEnum.AGAIN_REPORT_ISSUED_FAILED && item.id !== 0);
    if (errFlow) {
      keep = [errFlow];
    } else {
      const notZero = group.find((item) => item.processStatus === ProcessStatusEnum.AGAIN_REPORT_ISSUED && item.id !== 0);
      if (notZero) {
        keep = [notZero];
      } else {
        const pendingZero = group.find((item) => item.processStatus === ProcessStatusEnum.AGAIN_REPORT_PENDING && item.id !== 0);
        if (pendingZero) {
          keep = [pendingZero];
        } else {
          keep = [
            group.find((obj) => {
              return obj.processStatus === ProcessStatusEnum.AGAIN_REPORT_ISSUED;
            })!,
          ];
        }
      }
...
const isSecondReviewPassed = (item: sampleFlowListModel) => {
  ...
  if (isSecondReviewPassed && hasReportIssued) {
    ...
    // 2 是出具报告失败
    // 3是出具报告中
    if (item.processStatus === ProcessStatusEnum.REPORT_ISSUED_FAILED && item.id !== 0) {
      return 2;
    } else if (item.id !== 0) {
      return 1;
    } else {
...
function handleStepClick(item: sampleFlowListModel) {
  if (isSecondReviewPassed(item)) {
    // console.log("二审通过且未出具报告，可以出具报告1");

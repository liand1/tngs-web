...
import { findGeneResultListByName } from '@/api/lims/analysisgeneresult';
 ...
 * 获取靶区检测对比结果列表
 * @param taskId 任务ID
 * @param drugGene 病原体名称(中文名)
 * @returns 靶区检测对比结果列表
 */
export const getTagResultList = async (taskId: number, drugGene: string, sampleId: string) => {
  const res = await findGeneResultListByName(taskId, drugGene, sampleId);
  return res;
};
...
export async function handleExpand(expanded: boolean, record: any) {
  ...
  if (expanded) {
    ...
    if (!subDataMap.value[record.id]) {
      const children = await getTagResultList(record.taskId, record.drugGene, record.sampleId);
...
const getTableData = (data: any[]) => {
  ...
  subColumn.push({ title: "", dataIndex: "name", key: "name", width: 50 })
  ...
  data.forEach(item => {
    if (keysObj.value.current.includes(item.sampleCode)) {
      if(item.waterControls && item.waterControls.length > 0) {
        keysObj.value.water = keysObj.value.water.filter(waterSampleCode => item.waterControls?.includes(waterSampleCode));
      }
    }
    ...
    if (oneSubData.length <= 0) {
      subDatas.push({
        name, [item.sampleCode!]: item.mutationCount,
      })
    } else {
      oneSubData[0][item.sampleCode!] = item.mutationCount
    }

    ...
    <BasicTable @register="registerTable" bordered :pagination="false" v-model:expandedRowKeys="expandedRowKeys"
      ...
      <template #toolbar>
        <template v-if="isCurrentAudit || isCurrentAgain">
          <a-button v-auth="['lims:analysis-report:update']" @click="handleChangeByLabel(2)">
            转为主报告
          </a-button>
          <!-- <a-button v-auth="['lims:analysis-report:update']" @click="handleChangeByLabel(ByLabelEnum.SUSPECT)">
            转为疑似病原体
          </a-button> -->
          <a-button v-auth="['lims:analysis-report:update']" @click="handleChangeByLabel(1)">
            不在报告中展示
          </a-button>
        </template>
      </template>
...
import {
  ...
  copyAnalysisGeneResult,
  deleteAnalysisGeneResult,
} from "@/api/lims/analysisgeneresult";
import {
  mutationCodeType,
...
async function handleDelete(record: Recordable) {
  Modal.confirm({
    ...
    async onOk() {
      try {
        // 先调用删除API
        await deleteAnalysisGeneResult(record.id);
...
watch(
  () => props.findAnalysisReportDetails?.geneResultList,
  (newVal) => {
...
//复制
async function copyReport(record: any) {
  Modal.confirm({
    title: "操作确认",
    content: `确定要复制该条结果吗？`,
    okText: "确认",
    ...
    async onOk() {
      try {
        ...
        const newResult = {
          ...record,
          drugGene: record.drugGene + `（删除括号内内容）`,
        };
        // 使用传入的record，复制所有必要的字段
        await copyAnalysisGeneResult(newResult);
...
const handleChangeByLabel = async (label: number) => {
  ...
  try {
    ...
    createMessage.success(`设置${mutationCodeType[label]}成功`);
    ...
    createMessage.error(`设置${mutationCodeType[label]}失败，请重试`);
  }

  // Modal.confirm({
  //   title: "操作确认",
  //   content: `确定要将选中的${reportIds.value.length}个结果设置为${mutationCodeType[label]}吗？`,
  //   okText: "确认",
  //   cancelText: "取消",
  //   async onOk() {
  //     try {

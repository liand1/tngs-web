  ...
  <div>
    <div class="title">
      ...
      <div class="title">
        ...
        <a-button v-if="isCurrentAudit || isCurrentAgain" @click="editSample(findAnalysisReportDetails?.sample?.id || 0)" :loading="confirmEditLoading">编辑样本</a-button>
      </div>
    </div>
    ...
    <SampleModal @register="registerModal" :type="2" @success="updateSampleSuccess"/>
  </div>
</template>
...
const lightField = ["sample.sampleCode", "sample.sampleType", "sample.examinee.name", "sample.subHospital"];
...
const getNestedValue = (obj: any, path: string) => {
  ...
  if(path === 'sample.checkReagent') {
    return SampleCheckReagent[obj['sample']['checkReagent']];
  }
...
const updateSampleSuccess = (sampleData) => {
  console.log('sampleDataxxxxxx', sampleData);

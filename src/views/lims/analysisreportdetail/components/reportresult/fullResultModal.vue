<template>
  <a-modal v-model:open="open" title="分析结果报告" width="100%" 
    :showCancelBtn="true" @cancel="handleCancel" :footer="null" wrap-class-name="full-modal-reportResult">
    <template #closeIcon>
      <a-button>退出全屏模式
        <template #icon>
...
import {CopyOutlined, FileDoneOutlined, FullscreenExitOutlined, ReconciliationOutlined, ExceptionOutlined } from "@ant-design/icons-vue";
...
// 打开模态框方法
const showModal = async (reportId: number) => {
  open.value = true;
  nextTick(() => {
    arcRef.value?.setSelectIndex(reportId);
    handleNodeClick(reportId);
  })
};

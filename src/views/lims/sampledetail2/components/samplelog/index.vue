...
const getSampleLogList = async (batchCode: string, sampleId: string) => {
  const res = await getLimsLogPage({
    type: LimsLogTypeEnum.SAMPLE,
    bizId: sampleId || props.bizId,
    traceId: batchCode,
    pageNo: 1,
    pageSize: 100,
  });
...
const setSampleCode = (batchCode: string, sampleId: string) => {
  getSampleLogList(batchCode, sampleId);
};

...
// console.log(getDictOptions(DICT_TYPE.SYSTEM_USER_SEX))
...
export const userRoleFormSchema: FormSchema[] = [
  ...
  },
  {
    ...
    componentProps: {
      api: () => (async () => {
        const res = await listSimpleRoles()
        const userStore = useUserStore();
        const roles = userStore.getUserInfo?.roles || [];
        
        if (roles.some((role) => role === UserRoleEnum.SYSTEM_ADMIN)) {
          return res.filter((item) => item.code !== UserRoleEnum.SUPER_ADMIN && item.code !== UserRoleEnum.SYSTEM_ADMIN);
        }
        console.log(res)
        return res
      })(),

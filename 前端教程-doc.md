...
```bash
# --- 使用批处理脚本构建 (Windows)，批处理会同时打包好dev/test/prod环境三个tar包
右键运行，根目录下 ./build-docker-images.bat
```
...
```bash
# --- 单个镜像打包
docker build -t tngs-web-prod:1.0.3 .
```
...
# 导出镜像
docker save -o tngs-web-prod-1.1.0.tar tngs-web-prod:1.1.0

# 导入镜像
docker load -i tngs-web-prod-1.1.0.tar

# 运行容器
docker run -d -p 8091:80 --restart=always --name tngs-web-container-prod tngs-web-prod:1.1.0	--正式环境
